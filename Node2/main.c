...
uint8_t TxData[4] = {0};
uint8_t RxData[4] = {0};
volatile uint32_t rxFifoFillLevel;
...
int main(void)
{
  ...
  for (int i = 0; i < 4; i++) {
      ...
      sFilterConfig[i].FilterIndex = i;
      sFilterConfig[i].FilterType = FDCAN_FILTER_MASK;
      sFilterConfig[i].FilterConfig = FDCAN_FILTER_TO_RXFIFO0;
      sFilterConfig[i].FilterID1 = 0x111 + i;  // 0x111, 0x112, 0x113, 0x114
      sFilterConfig[i].FilterID2 = 0x7FF;
      ...
      for (uint32_t i = 0; i < fillLevel; i++)
      {
          if (HAL_FDCAN_GetRxMessage(&hfdcan1, FDCAN_RX_FIFO0, &RxHeader, RxData) == HAL_OK)
          {
              if (RxHeader.Identifier == 0x111)
              {
                  ...
                  __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, RxData[0]);
              }
              else if (RxHeader.Identifier == 0x112)
              {

                  __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, RxData[0]);
              }
              else if (RxHeader.Identifier == 0x113)
              {

                  __HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_1, RxData[0]);
              }
              else if (RxHeader.Identifier == 0x114)
              {

                  __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_1, RxData[0]);
              }
          }
      }
